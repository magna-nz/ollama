...
const (
	...
	// Clips longer than 30 s are encoded as independent 30 s chunks, one
	// soft-token run each; the reference extractor truncates at this
	// length instead.
...
func processAudio(data []byte) ([]audioChunk, error) {
	samples, rate, err := base.DecodeAudio(data)
	if err != nil {
		return nil, err
	}
	samples = base.Resample(samples, rate, audioSampleRate)
	...
	for off := 0; off < len(samples); off += audioChunkSamples {
		chunk := samples[off:min(off+audioChunkSamples, len(samples))]
