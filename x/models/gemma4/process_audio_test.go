...
func TestProcessAudioTokenCounts(t *testing.T) {
	cases := []struct {
		...
		{"chunk boundary plus minimum", 480161, []int{2999, 1}, []int{750, 1}},
		{"tail too short for a frame", 480100, []int{2999}, []int{750}},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, c := range chunks {
				if c.frames != tt.frames[i] || c.numTokens != tt.tokens[i] {
