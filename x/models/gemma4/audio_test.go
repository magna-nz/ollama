...
func TestPrepareAudioMediaChunks(t *testing.T) {
	...
	// 61 s: two full 30 s chunks of 750 tokens, then 25 for the last second,
	// all inside one boa/eoa pair.
	prepared, err := m.PrepareMedia([]base.Segment{{Kind: "audio", Data: wavPCM16(976000)}})
	if err != nil {
		t.Fatal(err)
	}
	if want := 1 + 750 + 750 + 25 + 1; len(prepared.Tokens) != want {
		t.Fatalf("%d tokens, want %d", len(prepared.Tokens), want)
	}
	if len(prepared.Items) != 3 {
		t.Fatalf("%d items", len(prepared.Items))
	}
	wantRanges := [][2]int{{1, 751}, {751, 1501}, {1501, 1526}}
	for i, item := range prepared.Items {
		if item.Range != wantRanges[i] || item.Source != 0 || !item.Causal {
